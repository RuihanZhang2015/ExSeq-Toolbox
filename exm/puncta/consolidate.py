...
def consolidate_channels_function(args, fov, code):
    ...
    for c in [1,2,3]:
        ...
        point_cloud1 = np.asarray([x['position'] for x in reference]) # Reference 640 channel puncta coordination 
        point_cloud2 = np.asarray(coords_total['c{}'.format(c)]) # other channels puncta coordination
        pairs = find_matching_points(point_cloud1,point_cloud2) # find closest pairs of puncta between 2 channels
    ...
    with h5py.File(args.h5_path.format(code,fov), 'r') as f:
        for i, duplet in enumerate(reference):
            ...
            duplet['color'] = np.argmax(temp) # Channel of the puncta with highest intensity
            duplet['intensity'] = temp # Intensity on different channels ['640','594','561','488']
            duplet['index'] = i #puncta index 
            duplet['position'] = duplet['c{}'.format(duplet['color'])]['position'] #postion of the highest intensity puncta
    ...
    def run(tasks_queue,q_lock):
        ...
        while True: # Check for remaining task in the Queue
            try:
                ...
                print('Consolidate Channels: Fov{}, Code{} Finished\n'.format(fov,code))        
...
def consolidate_codes_function(args, fov):
    ...
    for code in set(args.codes)-set([args.ref_code]):
        ...
        # create 
        point_cloud1 = np.asarray([x['position'] for x in reference]) # Reference puncta for code0 and other rounds 
        point_cloud2 = np.asarray([x['position'] for x in new]) # other rounds puncta  

        pairs = find_matching_points(point_cloud1,point_cloud2) # find closest pairs of puncta between 2 rounds
