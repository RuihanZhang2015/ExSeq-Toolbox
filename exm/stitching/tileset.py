...
class Tileset:
    ...
    def init_from_nd2(self, nd2):
        ...
        for i, raw_trans in enumerate(raw_translations):
            offsets.append(
                [raw_trans[b"dPosX"], raw_trans[b"dPosY"], raw_trans[b"dPosZ"]]
            )
    ...
    def preview_nd2(self, z_layer=0, down_sample="auto"):
        ...
        pictures = [
            self.get_slice(
                fovnum, z_layer + self.tiles[fovnum].offset[2] / self.voxel_size[2]
            )
            for fovnum in range(len(self.tiles))
        ]
        offsets = [t.offset[:2] / self.voxel_size[:2] for t in self.tiles]
        tile_size = np.array(pictures[0].shape)
        origin = np.min(offsets, axis=0)
        newshape = np.ceil(
            np.abs(np.max(offsets, axis=0) - origin) + np.array(tile_size)
        ).astype(np.uint)
        newpic = np.zeros(newshape[::-1], dtype=np.uint16)
        div = np.zeros(newshape[::-1], dtype=np.uint8)
        for off, tile in zip(offsets, pictures):
            newpic[
                int(off[1] - origin[1]) : int(off[1] - origin[1]) + tile_size[0],
                int(off[0] - origin[0]) : int(off[0] - origin[0]) + tile_size[1],
            ] += tile
            div[
                int(off[1] - origin[1]) : int(off[1] - origin[1]) + tile_size[0],
                int(off[0] - origin[0]) : int(off[0] - origin[0]) + tile_size[1],
            ] += np.ones(tile.shape, dtype=np.uint8)
        newpic = newpic / div
        if down_sample == "auto":
    ...
    def create_blank_h5bdv(
        self, target_file, tile_size=None, dtype=np.int16, noise_scale=200.0
    ):
        r"""
        ...
        To deactivate that put noise_scale at :math:`0.0`

        :param str target_file: Must be the path to the desired ``H5`` file. An XML file will be created at the same place
                                with the same name and the extension changed.

        :param tile_size: The size we want the tiles to be.
        :type tile_size: tuple(int)

        :param dtype: The type to initialize the arrays with.
        :type dtype: numpy.dtype

        :param float noise_scale: The intensity of the white noise the tiles are initialized with.
        ...
        bdv_writer = npy2bdv.BdvWriter(
            f"{target_file}", ntiles=len(self.tiles), overwrite=True
        )
    ...
    def find_intensity_scale(self, progress=False):
        r"""
        Finds the :math:`1^{st}` and :math:`99^{th}` percentile of intensity values in the dataset. Can be ``long``.
        """
    ...
    def scale_intensity(self, p1p99=None, progress=False):
        ...
        :param p1p99: lowest and highest value between which to scale. If ``None``, the function will
                                 calculate the :math:`1^{st}` and :math:`99^{th}` percentiles of intensities and use these.
        :type p1p99: tuple(int, int)

        :param bool progress: If ``True``, displays the progress of the percentile calculation (which is long)
        """
        if p1p99 is None:
    ...
    def write_into_h5bdv(self, filename, dtype=np.int16):
        r"""Exports the tileset as a H5BDV file format. It will create a ``.h5`` file containing the image information
        ...
        bdv_writer = npy2bdv.BdvWriter(
            f"{filename}", ntiles=len(self.tiles), overwrite=True
        )
    ...
    def show_slice(self, zslice, down_sample="auto"):
        r"""
        Shows a single :math:`Z`-slice of the stitched tileset.

        :param int zslice: :math:`z`-index to display
        :param down_sample: integer factor by which to downsample each dimension of the result. If
                                                 set to ``"auto"``, will make the biggest dimension as close as possible to
                                                 :math:`1500`
        :type down_sample: list[int, int], optional
        ...
        tile_size = np.array(pictures[0].shape)
        newshape = np.ceil(
            np.abs(np.max(offsets[:, :2], axis=0) - origin[:2]) + np.array(tile_size)
        ).astype(np.uint)
        newpic = np.zeros(newshape[::-1], dtype=np.uint16)
        div = np.zeros(newshape[::-1], dtype=np.uint8)
        for off, tile in zip(offsets, pictures):
    ...
    def update_offsets(self, xml_file, scale_factor=None):
        ...
        :param str xml_file: the file to read new offsets from.
        :param numpy.ndarray scale_factor: the vector by which to multiply offsets if necessary, defaults to ``None``. Usually unnecessary
        """
        ...
        offsets = np.array(stitching.get_offsets(xml_file)) * (
            self.voxel_size * np.array(scale_factor)
        )
    ...
    def local_to_global(self, coords):
        ...
        :param numpy.ndarray coords: Array of shape :math:`(n,4)` describing :math:`n` points. The rows are ordered in the :math:`(i, x, y, z)`
                                  order, :math:`i` being the index of the tile the point belongs to and :math:`(x, y, z)` its coordinates
                                  in :math:`\mu m` within the tile.
        :return: Array of shape :math:`(n,3)` containing the transformed coordinates in :math:`\mu m`
        :rtype: numpy.ndarray
        """
    ...
    def dedup_segmentation_ids(self, progress=False):
        ...
        new_ids = stitching.deduplicate_blob_ids(
            [t.img for t in self.tiles],
...
class Tile:
    ...
    def find_intensity_scale(self):
        r"""
        Returns the value of the first and :math:`99^{th}` percentile of intensities in the tile
        """
    ...
    def from_nd2(self, nd2, downsample=[1, 1, 1]):
        ...
        :param str nd2: can be a filename or an existing ND2Reader
        :param int fovnum: the FOV (along the :math:`v` axis) to extract
        :param downsample: a tuple of :math:`3` items describing a stride to apply in the :math:`(x, y, z)` axes, resulting in a downscale along these axes
        :type downsample: tuple(int)

        :return: An initialized tile with image and offset information
        :rtype: tileset.Tile
        """
        if type(nd2) is str:
            nd2 = ND2Reader(nd2)
    ...
    def fast_nd2_read_downscale(self, nd2, downscale):
        ...
        :param downscale: steps to apply while reading the image, in :math:`(z, y, x)` order
        :type downscale: list(int)
        """
        w = nd2.sizes["x"]
        h = nd2.sizes["y"]
        sz = w * h
        img = np.zeros(
            (
                round(nd2.sizes["z"] / downscale[0]),
                round(nd2.sizes["y"] / downscale[1]),
                round(nd2.sizes["x"] / downscale[2]),
            ),
            dtype=np.uint16,
        )
